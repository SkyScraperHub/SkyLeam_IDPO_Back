...
class SessionAdd(APIView):
    ...
    def post(self, request):
        ...
        if serializer_class.is_valid(raise_exception=True):
            MinioClient.upload_data(
                "public/" + str(request.auth["id"]) + "/" + file_name,
                file,
                length=file.size,
            )
...
@swagger_auto_schema(
    ...
    manual_parameters=[
        openapi.Parameter(
            "pk", openapi.IN_QUERY, type=openapi.TYPE_INTEGER, description="Document ID"
        )
    ],
    responses={
        200: "OK",
...
def DocGenerate(request):
    ...
    return FileResponse(
        io.BytesIO(pdf_form.stream),
        filename=f"sertificat_{session.FK_user.id}_{date}.pdf",
    )
...
class SessionVideoView(APIView):
    ...
    def get(self, request, pk):
        ...
        if session.video:
            video_url = MinioClient.get_presigned_url(
...
class UniqueScenariosSessionList(APIView):
    ...
    def get(self, request):
        try:
            unique_scenarios = (
                Session.objects.filter(FK_user__pk=request.auth["id"])
