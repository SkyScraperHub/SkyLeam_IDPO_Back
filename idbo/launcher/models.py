...
def validate_file_extension(value):
    ...
    if not ext.lower() in valid_extensions:
        raise ValidationError(
            f'Поддерживается следующий формат файла:{",".join(valid_extensions)}'
        )
...
def validate_version_format(value):
    ...
    if not pattern.match(value):
        raise ValidationError(
            "%(value)s is not a valid version format", params={"value": value}
        )
...
class GameImage(models.Model):
    ...
    img = models.ImageField(
        upload_to=upload_to, null=False, verbose_name="Название файла запуска"
    )
